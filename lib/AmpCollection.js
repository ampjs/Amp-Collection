    ...
    _createClass(AmpCollection, [{
        ...
        value: function _strictSchema(data) {
            ...
            if (JSON.stringify(dataKeys) !== JSON.stringify(this._schema)) {
                console.log('Collection:', 'Schema in strict mode -- keys do not match. Expecting:', this._schema, 'given;', dataKeys);
                window.stop();
            }
        ...
        value: function _nonStrictSchema(data) {
            for (var key = 0; key < this._schema.length; key++) {
                if (!data.hasOwnProperty(this._schema[key])) {
                    console.log('Collection:', 'key "' + this._schema[key] + '" missing from collection.');
                }
            }
        ...
        key: 'paginate',
        value: function paginate(count) {
            return new PagedCollection(this.processed, count);
        }
